 ...
 *
 * @details
 * This header provides the API for the debug framework.
 *
 * Features:
 *  - Multiple log severity levels (ERROR, WARN, INFO, DEBUG)
 *  - Formatted logging using printf-style functions
 *  - Optional sequence numbers and timestamps
 *  - Thread-safe output via abstracted transport and port layers
 *
 * The framework is portable across bare-metal and RTOS-based systems
...
/* Debug disabled: macros compile to no-ops */
#define LOG_ERROR(...)
#define LOG_WARN(...)
#define LOG_INFO(...)
#define LOG_DEBUG(...)
 ...
 *
 * @note
 * The transport HAL must provide a valid write() function.
 *---------------------------------------------------------------------------*/
